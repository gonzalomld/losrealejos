...
import { getArea, type Area } from "@/data/areas";
import type { Tramite } from "@/data/tramites";
import { Phone, Mail, MapPin } from "lucide-react";

export async function generateStaticParams() {
  const TODAS = await leerParaFront<Area>("areas");
  return TODAS.map((a) => ({ id: a.id }));
}
...
export default async function FichaArea({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!area) notFound();
  const TRAMITES = await leerParaFront<Tramite>("tramites");
  const tramites = area.tramiteIds.map((tid) => TRAMITES.find((x) => x.id === tid)).filter((x): x is Tramite => Boolean(x));
  return (
    <>
      <Breadcrumbs migas={[
