...
import { NOTICIAS, getNoticia, ETIQUETAS_CATEGORIA_NOTICIA } from "@/data/noticias";
...
export default async function NoticiaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!noticia) notFound();
  const relacionadas = NOTICIAS.filter((n) => n.id !== noticia.id && n.categoria === noticia.categoria).slice(0, 3);
  return (
    <>
