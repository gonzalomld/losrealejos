...
import { SERVICIOS_BARRIO, getServicioBarrio } from "@/data/servicios-barrio";
import { getArea } from "@/data/areas";
import { formatearFechaES } from "@/lib/formato";
import { ExternalLink, Phone } from "lucide-react";

export function generateStaticParams() {
  return SERVICIOS_BARRIO.map((s) => ({ id: s.id }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const s = getServicioBarrio(id);
  return s
...
export default async function ServicioBarrioPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const servicio = getServicioBarrio(id);
